...
class Solver(object):
    def __init__(self, train_loader, test_loader, config):
        ...
        self.build_model()
        if config.mode in ['test','newtest']:
    ...
    def test(self):
        ...
        for i, data_batch in tqdm(enumerate(self.test_loader)):
            ##################################################
            # images, names = data_batch['image'], data_batch['name']
            ...
            #     pred = np.squeeze(torch.sigmoid(preds[j]).cpu().data.numpy())
            #     image = np.squeeze(data_batch['origin'][j].cpu().data.numpy())
            #     mask = pred
            ...
            with torch.no_grad():
                ...
                image = np.squeeze(data_batch['origin'].cpu().data.numpy())
                mask = pred

                # image是图像，mask是掩码
                # 修改image，mask作为遮罩，遮罩的数字大小表示修改程度，数字为0表示image原图不保留，改为白色，数字为1表示image原图保留
                # 77.02G ########################
                # 74.86
                # 78.80
                # 
                ...
                for kk in range(255):
                    image[mask == kk] = (image[mask == kk]/255)*(kk) + np.array([1, 1, 1]).astype(np.uint8)*(255-kk)

                # cv2.imwrite(os.path.join(self.config.test_fold, names[j][:-4]+'.jpg'), image)

                cv2.imwrite(os.path.join(self.config.test_fold, name[:-4]+'.jpg'), image)
                # cv2.imwrite(os.path.join(self.config.test_fold, "mask_"+name[:-4]+'.jpg'), mask)
