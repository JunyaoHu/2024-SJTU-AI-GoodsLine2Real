...
def get_test_info(sal_mode='e'):
    if sal_mode == 'e':
        ...
        image_root = '/home/u1120230288/projects/2024-交大电商-AI绘制实体商品图/code/log_valid/1015-2017/Realistic Drawing'
        image_source = None
...
def main(config):
    if config.mode == 'train':
        ...
        test.test()
    elif config.mode == 'newtest':
        config.test_root, config.test_list = get_test_info(config.sal_mode)
        test_loader = get_loader(config, mode='newtest')
        if not os.path.exists(config.test_fold): os.mkdir(config.test_fold)
        test = Solver(None, test_loader, config)
        test.test()
    else:
...
if __name__ == '__main__':
    ...
    # Misc
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'test', 'newtest'])
