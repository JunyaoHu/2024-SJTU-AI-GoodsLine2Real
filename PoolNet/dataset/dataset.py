...
class NewImageDataset(data.Dataset):
    def __init__(self, folder_path, transform=None):
        """
        初始化数据集
        :param folder_path: 包含图片的文件夹路径
        :param transform: 应用于图片的转换操作
        """
        self.folder_path = folder_path
        self.transform = transform
        self.image_files = sorted([f for f in os.listdir(folder_path) if f.endswith('.jpg')])
        # 这里是为了多开推理
        # self.image_files = sorted([f for f in os.listdir(folder_path) if f.endswith('.jpg')])[:5000]
        # self.image_files = sorted([f for f in os.listdir(folder_path) if f.endswith('.jpg')])[5000:]
    ...
    def __getitem__(self, idx):
        ...
        return {'origin':im , 'image': in_, 'name': self.image_files[idx], 'size': im_size}
...
def get_loader(config, mode='train', pin=False):
    ...
    if mode == 'newtest':
        ...
        dataset = NewImageDataset(config.test_root, transform)
        print(len(dataset))
        data_loader = data.DataLoader(dataset=dataset, batch_size=config.batch_size, shuffle=shuffle, num_workers=config.num_thread, pin_memory=pin)
    return data_loader
